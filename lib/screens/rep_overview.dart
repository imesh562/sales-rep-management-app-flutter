...
import 'package:dk_brothers/components/rep_overview_pie.dart';
...
import 'package:dk_brothers/screens/today_attendance.dart';
...
class RepOverview extends StatefulWidget {
  static String id = 'rep_overview';
  @override
  _RepOverviewState createState() => _RepOverviewState();
}

class _RepOverviewState extends State<RepOverview> {
  ...
  waitingList() async {
    ...
    await obj1.getDataRepPie();
    await obj1.getRepNames();
    await targetCal();
    if (this.mounted)
      setState(() {
                      ...
                      onTap: () {
                        ...
                        pushNewScreenWithRouteSettings(
                          ...
                          settings: RouteSettings(
                            name: TodayAttendance.id,
                          ),
                          screen: TodayAttendance(finalDate),
                          withNavBar: true,
          ...
          if (targetResults.containsKey(repID)) {
            if (discountedTotal < total && discountedTotal > 0) {
              targetResults.update(
                  repID, (dynamic val) => val + discountedTotal);
            } else {
              targetResults.update(repID, (dynamic val) => val + total);
            }
          } else {
            if (discountedTotal < total && discountedTotal > 0) {
              targetResults[repID] = discountedTotal;
              shopsMap[repID] = repName;
            } else {
              targetResults[repID] = total;
              shopsMap[repID] = repName;
            }
          }
        }
